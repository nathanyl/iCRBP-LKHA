from keras.layers import Dense, Convolution1D, Dropout, Input, Activation, Flatten,MaxPool1D,add, AveragePooling1D, Bidirectional,GRU,LSTM,Multiply,Activation, MaxPooling1D,TimeDistributed,AvgPool1D,nn,Depth_Conv2d,DepthWiseConv2d,DropPath,
...
class RepLKBlock(nn.Module):
    ...
    def __init__(self, kernel_size, c_in, prob):
        super(RepLKBlock, self).__init__()
        
        self.bn = nn.BatchNorm2d(c_in)
        self.conv1 = nn.Conv2d(c_in, kernel_size=2, stride=2, bias=False)
        self.conv_DC1 = Depth_Conv2d(c_in,kernel_size=2, stride=2, bias=False)
        self.conv_dw = DepthWiseConv2d(c_in, stride=1, kernels_per_layer=8, kernel_size=kernel_size)
        self.conv2 = nn.Conv2d(c_in,  kernel_size=1, stride=1,bias=False)
        self.drop_path = DropPath(prob)
        self.conv3= nn.Conv2d(c_in,  kernel_size=7, stride=1, bias=False,MaxPool1D=c_in)
        self.conv4= nn.Conv2d(c_in,  kernel_size=5, stride=1, bias=False,MaxPool1D=c_in)
        self.conv5= nn.Conv2d(c_in,  kernel_size=3, stride=1, bias=False,MaxPool1D=c_in)
        self.conv6= nn.Conv2d(c_in,  kernel_size=1, stride=1, bias=False,MaxPool1D=c_in)
...
def main(parser):
    ...
    for train_index, eval_index in kf.split(train_label):
        ...
        print('\n Testing accuracy of bdt_real_test_CNN (Replknet+CNN): {}'.format(
